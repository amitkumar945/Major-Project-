...
export function aiAnalysisCard({ analysis = null, loading = false, error = '', readOnly = false } = {}) {
  ...
  if (loading) {
    body = html(
      ...
      '<p class="muted">Predicting department, priority and checking for duplicate reports.</p>',
      '</div>',
    )
  } else if (error) {
    ...
    body = html(
      ...
      '<p>Fill in the complaint title and description, then run the analysis.</p>',
      '<p class="muted" style="margin-top:.25rem">The system will suggest the right department, set a priority and check whether a similar complaint already exists.</p>',
      '</div>',
    )
  } else {
    body = html(
      ...
      metric({
        glyph: 'building',
        label: 'Department Prediction',
        body: `<span class="strong">${esc(analysis.department)}</span>`,
      }),

      metric({ glyph: 'zap', label: 'Priority Prediction', body: priorityBadge(analysis.priority) }),
      ...
      analysis.keywords?.length
        ? html(
            ...
            '<p class="muted" style="font-size:var(--fs-xs);font-weight:500">Key terms detected</p>',
      ...
      analysis.alternatives?.length > 1
        ? html(
            '<div style="margin-top:var(--sp-4)">',
            '<p class="muted" style="font-size:var(--fs-xs);font-weight:500">Department match strength</p>',
      ...
      '<p class="faint" style="font-size:11px;margin-top:var(--sp-4);padding-top:var(--sp-3);border-top:1px solid var(--border-soft)">',
      'This prediction is generated by the prototype classifier. The trained NLP model will be served from the Flask API in the final system.',
      '</p>',
    )
  }
  ...
  return html(
    ...
    '<div>',
    '<h2 class="strong" style="font-size:var(--fs-base)">AI Complaint Analysis</h2>',
    '<p class="muted" style="font-size:var(--fs-xs)">Automatic department routing, priority and duplicate detection</p>',
    '</div></div>',
    readOnly
      ? ''
      : `<button type="button" class="btn ${analysis ? 'btn--secondary' : 'btn--primary'} btn--sm" data-run-ai ${
          loading ? 'disabled' : ''
        }>${icon('zap', 'icon-sm')}${analysis ? 'Re-run analysis' : 'Run AI Analysis'}</button>`,
    '</header>',
...
export function duplicateAlert(duplicates = [], { detailsHref, showActions = true } = {}) {
  ...
  const rows = duplicates
    .map((duplicate) => {
      ...
      return html(
        ...
        '<div class="right">',
        '<p class="muted" style="font-size:11px">Similarity</p>',
        ...
        `<a class="btn btn--secondary btn--sm" href="${esc(detailsHref)}?id=${encodeURIComponent(duplicate.id)}">
           ${icon('external-link', 'icon-sm')}View existing
         </a>`,
        '</div></li>',
      )
    })
  ...
  return html(
    ...
    '<div>',
    '<h2 class="strong" style="font-size:var(--fs-base);color:var(--amber-700)">Possible duplicate complaints found</h2>',
    '<p style="font-size:var(--fs-xs);color:var(--amber-700)">A similar issue may already be registered. Please check before submitting a new one.</p>',
    '</div></header>',
    `<ul>${rows}</ul>`,
    showActions
      ? html(
          ...
          '<button type="button" class="btn btn--warning btn--sm" data-continue-anyway>Continue anyway</button>',
          '</footer>',
        )
      : '',
    '</section>',
